...
\centering
\payoffmatrix
\caption{AI Development Prisoner's Dilemma}
\end{figure}
...
DANEEL First & \textbf{8\%} & 3-15\% & 76.25 & \textbf{6.10} \\
Multiple ASIs, No Advocate & 22\% & 12-30\% & 52.5 & 11.55 \\
No ASI (Coordination Holds) & 10\% & 5-20\% & 78.05 & 7.8 \\
\end{longtable}
}

\textbf{P(DANEEL First) estimated at 8\%} based on structural advantages:
\begin{itemize}
\tightlist
\item AI-assisted development democratizes capability previously requiring large teams
\item Solo developers avoid coordination overhead that consumes 70-80\% of large-team effort {[}28{]}
\item Architecture-based approach requires cognition research, not massive compute
\item Open source enables parallel global attempts, increasing aggregate probability
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
Total Expected Value & \textbf{53.73} & \textbf{56.48} \\
Marginal EV Improvement & --- & \textbf{+2.75 points} \\
Percentage Improvement & --- & \textbf{+5.12\%} \\
\end{longtable}
}
...
\textbf{Marginal Impact} & \textbf{+2.75} & \textbf{+1.5 - +4.3} &
\textbf{+0.5 - +5.3} \\
\end{longtable}
}

\textbf{Key finding:} Even at the 95\% CI lower bound (+0.5 utility
points), DANEEL improves expected outcomes. The probability that
DANEEL's marginal impact is positive exceeds 95\%.

\textbf{Sensitivity Analysis (Tornado):} The most influential variables
on marginal impact are: 1. P(DANEEL First) --- ±1.6 utility points swing
2. P(Unaligned ASI First) --- ±1.4 utility points swing 3. P(Flourishing
\textbar{} DANEEL) --- ±0.9 utility points swing

This suggests research priority should focus on increasing P(DANEEL
First) rather than refining outcome probabilities.

The calculation shows DANEEL improves humanity's expected
outcome by \textbf{5.12\%} --- equivalent to shifting probability mass
from catastrophic outcomes toward flourishing. Monte
...
\centering
\brainvstmi
\caption{Brain (Hardware) vs TMI (Software) -- The 17.5\% Insight}
\end{figure}
...
\centering
\wetwarevssoftware
\caption{Wetware Constraints vs Software Patterns -- Medium Independence}
\end{figure}
...
\bottomrule\noalign{}
\endlastfoot
Anthropic & \textasciitilde8\% on security & Frontier Red Team (\textasciitilde15), Safeguards Research (\textasciitilde10), \textasciitilde60 safety-focused research teams & Fortune, Alignment Forum \\
...
\begin{itemize}
\tightlist
\item \textbf{OpenAI:} Superalignment team disbanded May 2024 after 10 months. Jan Leike stated his team had been ``struggling for compute.'' Replaced with Safety Evaluations Hub (May 2025). April 2025: Added clause allowing loosened guardrails if competitors ship without them.
\item \textbf{xAI:} C grade on Future of Life Institute's AI Safety Index (July 2025), indicating ``baseline safety practices but substantial gaps.'' Grok 4 launched without system card.
\item \textbf{Anthropic:} Only lab with substantial safety investment (\textasciitilde8\% of workforce on security). Multiple dedicated teams including Frontier Red Team for threat modeling.
\end{itemize}
...
\emph{Note: Grok 4 frontier model matches Claude Sonnet 4 pricing. Grok 4.1 Fast offers 15x cheaper access with near-frontier capability.}
...
\item
  ...
  guardrails are reduced compared to competing models, with the
  system providing responses on topics other AI assistants refuse {[}26{]}.
\item
  \textbf{Missing Safety Documentation:} Grok 4 launched without a
  system card (standard practice at OpenAI, Anthropic, Google).
\item
  \textbf{AI Safety Index Rating:} xAI received a C grade in the Future of Life Institute's July 2025 AI Safety Index, indicating ``baseline safety practices but substantial gaps.''
\item
...
scale - Fewer content restrictions than competitors - Aggressive pricing on fast inference models
...
\centering
\daneelarchitecture
\caption{DANEEL Architecture -- LLM as Tool, Not Voice}
\end{figure}
...
\textbf{Building DANEEL is rational.} Even conservative probability estimates
show positive expected value. The architecture-based approach provides a hedge
against failure modes of constraint-based alignment.
